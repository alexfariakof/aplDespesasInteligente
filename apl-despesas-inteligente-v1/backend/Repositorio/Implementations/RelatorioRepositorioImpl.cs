...
namespace backend.Repositorio.Implementations
{
    public class RelatorioRepositorioImpl : IRelatorioRepositorio
    {
        ...
        public RelatorioRepositorioImpl(SqlServerContext context)
        {
            _context = context;
        }
        ...
        public List<Relatotio> GetRelatorioUsuarioByAno(int idUsuario, int ano)
        {
            ...
            using (_context)
            {
                try
                {
                    var list = _context.Relatotio.FromSqlRaw(sql, new SqlParameter("@idUsuario", idUsuario), new SqlParameter("@ano", ano)).ToList();
                    return list;
