    ...
    public class LancamentoRepositorioImpl : ILancamentoRepositorio
    {
        ...
        public List<Lancamento> FindByMesAno(DateTime data, int idUsuario)
        {
            ...
            using (_context)
            {
                try
                {
                    var list = _context.Lancamento.FromSqlRaw(sql, new SqlParameter("@idUsuario", idUsuario), new SqlParameter("@mes", data.Month), new SqlParameter("@ano", data.Year)).ToList();
